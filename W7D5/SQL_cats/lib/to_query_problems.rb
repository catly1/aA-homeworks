...
def price_like_shiny_mouse
  ...
  FROM
    toys
  JOIN
    toys AS toys2 ON toys.price = toys2.price
  WHERE
    toys2.name = 'Shiny Mouse' AND
...
def just_like_orange
  ...
  FROM
    cats
  JOIN
    cats AS cats2 ON cats.breed = cats2.breed 
  WHERE
    cats2.name = 'Orange' AND cats.name != 'Orange'
  ORDER BY
    cats.name; 
  SQL
end
...
def toys_that_jet_owns
  ...
  execute(<<-SQL)
  SELECT DISTINCT 
    not_jet_cat.name, toys.name
  FROM
    toys
  JOIN
    cattoys AS jet_toys ON jet_toys.toy_id = toys.id
  JOIN
    cats AS jet_cat ON jet_toys.cat_id = jet_cat.id
  JOIN
    cattoys AS not_jet_toys ON jet_toys.toy_id = not_jet_toys.toy_id
  JOIN
    cats AS not_jet_cat ON not_jet_cat.id = not_jet_toys.cat_id
  WHERE
    jet_cat.name = 'Jet' AND
    not_jet_cat.name != 'Jet'
  ORDER BY
    not_jet_cat.name;
  SQL
end
